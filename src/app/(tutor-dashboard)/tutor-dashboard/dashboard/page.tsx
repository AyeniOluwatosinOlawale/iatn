...
export default async function TutorDashboardPage({
  ...
  type VisitorRow = { viewer_name: string | null; viewer_role: string; created_at: string }
  ...
  if (tutor?.id) {
    const bookingsRes = await (supabase
      ...
      .limit(3) as unknown as Promise<{ data: BookingRow[] | null }>)
    bookings = bookingsRes.data ?? []

    const visitorsRes = await (supabase
      .from('profile_views')
      .select('viewer_name, viewer_role, created_at')
      .eq('tutor_id', tutor.id)
      .order('created_at', { ascending: false })
      .limit(10) as unknown as Promise<{ data: VisitorRow[] | null }>)
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-bold text-slate-900">Profile Visitors</h2>
              <span className="text-xs text-slate-400">Who viewed your profile</span>
            </div>
            {visitors.length > 0 ? (
              <div className="space-y-3">
                {visitors.map((v, i) => (
                  <div key={i} className="flex items-center gap-3 py-2 border-b border-slate-50 last:border-0">
                    <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600 flex-shrink-0">
                      {v.viewer_name ? v.viewer_name[0].toUpperCase() : '?'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-semibold text-slate-800 truncate">{v.viewer_name ?? 'Anonymous'}</div>
                      <div className="text-[10px] text-slate-400 capitalize">{v.viewer_role}</div>
                    </div>
                    <span className="text-[10px] text-slate-400 flex-shrink-0">
                      {new Date(v.created_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'short' })}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div className="text-center py-10 text-slate-400">
                ...
                <p className="text-xs mt-1">Students and parents who view your profile will appear here.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { icon: Eye, label: 'View Public Profile', href: tutor?.id ? `/tutors/${tutor.id}` : '/tutors/me', color: 'bg-blue-50 text-blue-700' },
